...
struct StepHistoryView: View {
    @ObservedObject private var dataManager = StepDataManager.shared
    
    var body: some View {
        NavigationView {
            Group {
                if dataManager.stepHistory.isEmpty {
                    VStack(spacing: 12) {
                    ...
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                        ...
                        List {
                            ForEach(dataManager.stepHistory.sorted(by: { $0.date > $1.date })){ record in
                                HStack {
    ...
    private func stepDifference(for record: StepRecord) -> Double? {
        guard let index = dataManager.stepHistory.firstIndex(where: { $0.id == record.id }),
              index + 1 < dataManager.stepHistory.count else {
            return nil
        }
        let previous = dataManager.stepHistory[index + 1]
